...
스트라센 알고리즘은 이름 그대로 독일의 수학자 폴커 스트라센(Volker Strassen)이 만든 행렬 곱 계산을 빠르게 수행하기 위해 만든 알고리즘이다.
...
그리고 이 행렬곱의 수행시간은 행렬곱인 C의 nxn개의 원소에서 n번에 곱셈을하므로 n의 3제곱의 수행시간을 가지고 있다.

#### 3.스트라센 알고리즘 

스트라센 알고리즘은 이 행렬곱을 곱셈을 조금 더 사용하지 않고 계산하기 위해 만들어 졌다.
...
만약 이 곱셈을 덜 하고 덧셈을 더 하는 것을 반복하여 식으로 나타내면 ![{\displaystyle 7\cdot n^{\log _{2}7}-6\cdot n^{2}}](https://wikimedia.org/api/rest_v1/media/math/render/svg/586e85e6ba93daf1db18e144da90a79af278e9a9) 번의 연산이 필요한 것으로 나온다,

여기서  log2의7=2.807 이므로 총 연산시간은 n의 약2.8 제곱이다.

스트라센 알고리즘은 행렬곱 계산에서 참신한 방법으로 연산시간을 n의 3제곱에서 더 줄여준다.

하지만 이 스트라센 알고리즘이 컴퓨터에서 행렬 곱 계산에 쓰일 때는 M이라는 수식을 만들어 저장해야할 공간도 있어야 하고 n의 숫자가 작을 때는 위에서 볼때 와 같이 식에 덧셈이 많아져 연산시간이 더 걸려 비효율적일 수 있다.

하지만 n의 숫자가 많아질수록 점차 스트라센의 알고리즘의 연산시간이 일반적인 행렬 곱 계산보다더 적어지므로 큰 행렬끼리의 곱을 계산 할 때 더 시간을 단축시킬 수 있다. 또한 컴퓨터로 행렬 곱 계산을 시행할 때에는 대체적으로 n의 큰 경우가 많으므로 이 알고리즘은 전체적으로 효율적인 알고리즘이라고 볼 수 있다.
