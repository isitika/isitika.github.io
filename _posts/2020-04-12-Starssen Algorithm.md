...
하지만 n의 숫자가 많아질수록 점차 스트라센의 알고리즘의 연산시간이 일반적인 행렬곱연산보다더 적어지므로 큰 행렬끼리의 곱을 계산 할 때 더 시간을 단축시킬 수 있다. 또한 컴퓨터로 행렬의 곱연산을 시행할 때에는 대체적으로 n의 큰 경우가 많으므로 이 알고리즘은 전체적으로 효율적인 알고리즘이라고 볼 수 있다.
